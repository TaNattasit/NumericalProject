...
export default class Bisection extends Component
{
  constructor(props)
    ...
    componentDidMount()
    {
        // axios.get('http://localhost/Numer/server/show_Bi.php')
        axios.get('http://localhost:8080/show_Bi.php')
        .then(res=>{
...
getValue = ()=>{
  ...
  EquationBisection = () =>{
    ...
    formData.append("E_Type","Bisection");
    ...
    axios.post('http://localhost:8080/insertdata.php',formData,config)
    .then(res=>{
      console.log(res);
    })
    .catch(err=>{
      ...
      showResult=()=>{
        ...
        {
          return <div>
            <h5 style={{fontSize:25}}>This is Result of Bisection : {this.state.result}</h5><br/>
